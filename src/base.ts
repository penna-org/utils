...
export const isDef = <T = any>(val?: T | null): val is T => typeof val !== 'undefined' && !isNull(val)
...
export function isBaseType(val: unknown): val is string | number | boolean | symbol | undefined | null {
...
export function pick<O, K extends keyof O, R extends Record<string, any>>(obj: O, keys: K[] = [], rest?: R): Pick<O, K> & (R extends undefined ? NonNullable<unknown> : R) {
  const newObj: any = {}
  const names: K[] = Object.getOwnPropertyNames(obj) as K[]
  names.forEach((name) => {
    if ((keys as string[]).includes(name as string))
      newObj[name] = obj[name]
  })
  return Object.assign(newObj, rest)
}

export function omit<T, K extends keyof T, R extends Record<string, any>>(object: T, keys: K[] = [], rest?: R): Omit<T, K> & (R extends undefined ? NonNullable<unknown> : R) {
